...
//	ret = (err == OS_NO_ERR);
    ...
    OS_ERR err;
	OSMutexCreate (sobj, "fsMtx", &err);      /* uC/OS-III */
...
//	ret = (err == OS_NO_ERR);
	...
	OS_ERR err;
	OSMutexDel(&sobj, OS_OPT_DEL_NO_PEND, &err); /* uC/OS-III */
	return (err == OS_ERR_NONE);
...
//	ret = (err == OS_NO_ERR);
    ...
    OS_ERR err;                                     /* uC/OS-III */
    OSMutexPend(&sobj, _FS_TIMEOUT, OS_OPT_PEND_BLOCKING, (void*)0, &err);
    return (err == OS_ERR_NONE);
